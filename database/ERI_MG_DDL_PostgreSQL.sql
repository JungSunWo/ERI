...
-- =================================================================
-- 9. 첨부파일 관리 뷰 (File Attachment Management Views)
-- =================================================================

-- 공지사항 첨부파일 뷰
CREATE VIEW V_NTI_FILE_ATTACH AS
SELECT 
    n.SEQ as NTI_SEQ,
    n.TTL as NTI_TTL,
    n.FILE_ATTACH_YN,
    ...
    fa.FILE_ORD,
    fa.DOWNLOAD_CNT,
    fa.REG_DATE as FILE_REG_DATE
FROM TB_NTI_LST n
LEFT JOIN TB_FILE_ATTACH fa ON n.SEQ = fa.REF_PK_VAL::BIGINT 
    AND fa.REF_TBL_CD = 'NTI' 
    AND fa.DEL_YN = 'N'
WHERE n.DEL_YN = 'N'
ORDER BY n.SEQ, fa.FILE_ORD, fa.REG_DATE;

-- 프로그램 첨부파일 뷰
CREATE VIEW V_PGM_FILE_ATTACH AS
SELECT 
    p.SEQ as PGM_SEQ,
    p.PGM_NM,
    p.FILE_ATTACH_YN,
    ...
    fa.REG_DATE as FILE_REG_DATE
FROM TB_PGM_LST p
LEFT JOIN TB_FILE_ATTACH fa ON p.SEQ = fa.REF_PK_VAL::BIGINT 
    AND fa.REF_TBL_CD = 'PGM' 
    AND fa.DEL_YN = 'N'
WHERE p.DEL_YN = 'N'
ORDER BY p.SEQ, fa.FILE_ORD, fa.REG_DATE;

-- 직원 첨부파일 뷰
CREATE VIEW V_EMP_FILE_ATTACH AS
SELECT 
    e.EMP_ID,
    e.EMP_NM,
    e.FILE_ATTACH_YN,
    ...
    fa.REG_DATE as FILE_REG_DATE
FROM TB_EMP_LST e
LEFT JOIN TB_FILE_ATTACH fa ON e.EMP_ID = fa.REF_PK_VAL 
    AND fa.REF_TBL_CD = 'EMP' 
    AND fa.DEL_YN = 'N'
WHERE e.DEL_YN = 'N'
ORDER BY e.EMP_ID, fa.FILE_ORD, fa.REG_DATE;

-- 첨부파일 통계 뷰
CREATE VIEW V_FILE_ATTACH_STATS AS
SELECT 
    fa.REF_TBL_CD,
    CASE 
        WHEN fa.REF_TBL_CD = 'NTI' THEN '공지사항 목록'
        WHEN fa.REF_TBL_CD = 'PGM' THEN '프로그램 목록'
        WHEN fa.REF_TBL_CD = 'EMP' THEN '직원 목록'
        WHEN fa.REF_TBL_CD = 'DEPT' THEN '부서 목록'
        WHEN fa.REF_TBL_CD = 'MNU' THEN '메뉴 목록'
        WHEN fa.REF_TBL_CD = 'AUTH' THEN '권한 목록'
        ELSE '기타'
    END as REF_TBL_NM,
    COUNT(fa.FILE_SEQ) as TOTAL_FILES,
    SUM(fa.FILE_SIZE) as TOTAL_SIZE,
    SUM(fa.DOWNLOAD_CNT) as TOTAL_DOWNLOADS,
    AVG(fa.FILE_SIZE) as AVG_FILE_SIZE,
    MAX(fa.REG_DATE) as LAST_UPLOAD_DATE
